...
Proof using TCCOH. cdes TCCOH; generalize ICOV; unfold D; basic_solver 12. Qed.

Lemma dom_addr_in_D : dom_rel Gaddr ⊆₁ D.
Proof using E_to_I TCCOH WF.
...
Proof using TCCOH WF WF_SC detour_Acq_E detour_E.
  unfold D.
  rewrite !id_union; relsf; unionL; splits.
  ...
  { rewrite (dom_r (wf_detourD WF)).
    rewrite (issuedW TCCOH) at 1. clear. type_solver. }
  { apply detour_E. }
  { rewrite dom_rel_eqv_dom_rel.
...
- rewrite (data_in_sb WF).
  ...
  arewrite (dom_rel (⦗D⦘ ⨾ Gsb ⨾ ⦗E ∩₁ NTid_ thread⦘) ⊆₁ D) by clear; basic_solver.
  unfold D. clear. clear; basic_solver 12.
- rewrite dom_rel_eqv_dom_rel.
  rewrite crE at 1; relsf; unionL; splits.
  rewrite (dom_r (wf_dataD WF)), (dom_l (@wf_ppoD G)); clear; type_solver.
  rewrite (data_in_ppo WF).
...
- arewrite (⦗dom_rel (Gsb^? ⨾ ⦗I⦘) ∩₁ F ∩₁ Acq/Rel⦘ ⊆ ⦗dom_rel (⦗F ∩₁ Acq/Rel⦘ ⨾ Gsb^? ⨾ ⦗I⦘)⦘).
  ...
  generalize (dom_F_sb_I_in_C TCCOH). clear; basic_solver 12.
  generalize (dom_sb_covered TCCOH); clear; basic_solver 21.
Qed.
...
arewrite (⦗GW_ex⦘ ⨾ ⦗E⦘ ⊆ ⦗GW_ex ∩₁ E⦘) by clear; basic_solver.
rewrite W_ex_E.
unfold D.
clear; basic_solver 21.
Qed.
...
unfold D.
rewrite !id_union; relsf; unionL; splits.
- ie_unfolder; generalize (dom_rf_covered WF TCCOH); clear; basic_solver 21.
...
- rewrite dom_rel_eqv_dom_rel.
  rewrite crE at 1; relsf; unionL; splits.
  2: by rewrite (dom_r (wf_rfiD WF)), (dom_l (wf_rfiD WF)); clear; type_solver.
  clear; basic_solver 12.
...
Proof using COMP_C COMP_NTID COMP_PPO TCCOH WF.
unfold D.
rewrite !set_inter_union_l.
unionL.
- apply COMP_C.
...
- ie_unfolder; clear; basic_solver.
- ie_unfolder; clear; basic_solver.
Qed.

Lemma dom_Rex_sb_in_D : dom_rel (⦗GR_ex⦘ ⨾ Gsb) ⊆₁ D.
Proof using E_to_I TCCOH.
...
- rewrite dom_rel_eqv_dom_rel.
  arewrite (⦗I⦘ ⊆ ⦗W⦘ ⨾ ⦗I⦘).
  { generalize (issuedW TCCOH); clear; basic_solver. }
  rewrite <- dom_eqv1.
  ...
  2: by unfold D; clear; basic_solver 21.
  unfold ppo; rewrite <- ct_step; clear; basic_solver 12.
Qed.

Lemma dom_Rex_fail_sb_in_D : dom_rel (⦗GR_ex \₁ dom_rel Grmw⦘ ⨾ Gsb) ⊆₁ D.
Proof using E_to_I TCCOH.
  rewrite <- dom_Rex_sb_in_D. clear. basic_solver.
Qed.

Lemma dom_ppo_D : dom_rel (Gppo ⨾ ⦗D⦘) ⊆₁ D.
Proof using E_to_I TCCOH WF W_ex_E.
...
relsf; unionL.
- rewrite (dom_rel_helper dom_data_D); sin_rewrite H; clear; basic_solver.
- rewrite (dom_rel_helper dom_ctrl_in_D); rewrite !seqA; sin_rewrite H; clear; basic_solver.
- rewrite (dom_rel_helper dom_addr_in_D); rewrite !seqA; sin_rewrite H; clear; basic_solver.
- rewrite (dom_rel_helper dom_rfi_D); sin_rewrite H; clear; basic_solver.
- rewrite (dom_rel_helper dom_rmw_in_D). rewrite !seqA. sin_rewrite H; clear; basic_solver.
- rewrite (dom_rel_helper dom_frmw_in_D); rewrite !seqA; sin_rewrite H; clear; basic_solver.
- rewrite (dom_rel_helper dom_Rex_sb_in_D); rewrite !seqA; sin_rewrite H; clear; basic_solver.
Qed.
...
Proof using E_to_I TCCOH WF W_ex_E.
  ...
  unionL.
  { rewrite dom_rmw_in_D. clear; basic_solver. }
  rewrite E_to_I.
  rewrite C_in_D, I_in_D. 
  generalize dom_Rex_fail_sb_in_D. clear; basic_solver 10.
Qed.
...
Proof using COH COMP_NTID G IT_new_co T TCCOH_rst_new_T WF W_hb_sc_hb_to_I_NTid detour_E.
unfold cert_co.
red; intros x y H.
assert (A: (E ∩₁ W ∩₁ Tid_ thread) y).
{ apply (co_for_split IT_new_co (wf_coE WF) (wf_coD WF)).
  red. eauto. }
unfolder in H; desf.
assert (B: (E ∩₁ W) x).
{ hahn_rewrite (wf_new_coE IT_new_co (wf_coE WF)) in H0.
  hahn_rewrite (wf_new_coD IT_new_co (wf_coD WF)) in H0.
  unfolder in H0. desc. unfolder. split; auto. }
apply IT_new_co in B; unfolder in B; desf.
unfolder in A; desf.
assert (D: (⦗ E ∩₁ W ∩₁ Tid_ (tid x) ⦘ ⨾ Gco) x y).
{ eapply T_new_co; try edone; try apply WF.
...
apply dom_helper_3.
unfold new_rf.
rewrite (wf_furrE WF WF_SC); clear; basic_solver 21.
Qed.
...
apply dom_helper_3.
unfold new_rf.
unfold furr, urr; clear; basic_solver.
Qed.

Lemma wf_new_rfl : new_rf ⊆ Gsame_loc.
Proof using.
unfold new_rf; clear; basic_solver.
...
Proof using IT_new_co WF WF_SC.
...
Proof using IT_new_co WF.
ins; unfolder in *; desf.
assert (exists l, Gloc b = Some l) by (by apply (is_r_loc Glab)); desc.
assert (E (InitEvent l)).
{ by apply WF; eauto. }
assert (Glab (InitEvent l) = Astore Xpln Opln l 0) by (by apply WF). 
...
{ exists (InitEvent l); splits.
  ...
  match goal with
  | H : Gsb (InitEvent l) b |- _ => generalize H
  end.
  clear; basic_solver 12. }
...
- ins; desf.
  ...
  * unfold furr, same_loc.
    unfolder in A; desf; unfolder; ins; desf; splits; try basic_solver 21.
    all: congruence.
  * unfold max_elt in *.
...
Proof using ACYC_EXT COH CSC WF WF_SC.
rewrite new_rf_in_furr.
apply furr_hb_sc_hb_irr; done.
Qed.

Lemma non_I_new_rf: ⦗E \₁ I⦘ ⨾ new_rf ⊆ Gsb.
Proof using ACYC_EXT COH COMP_NTID CSC IT_new_co TCCOH TCCOH_rst_new_T WF WF_SC
      W_hb_sc_hb_to_I_NTid detour_E.
  ...
  unfolder. intros x y [[[[[EX NIX] TIDX] WX] _] [NEWRF [[EY TIDY] RY]]].
  subst. eapply (@same_thread G) in TIDY; desf.
  2: { clear -RY WF. intro K. apply (init_w WF) in K. type_solver. }
  destruct TIDY as [|SB].
  { subst x. clear -RY WX. type_solver. }
  exfalso; generalize new_rf_hb SB NEWRF.
  generalize (@sb_in_hb G). clear. basic_solver 12.
Qed.

Lemma new_rfe_Acq : (new_rf \ Gsb) ⨾ ⦗R∩₁Acq⦘ ⊆ ∅₂.
Proof using ACYC_EXT COH COMP_ACQ COMP_NTID CSC G IT_new_co T TCCOH TCCOH_rst_new_T WF WF_SC
      W_hb_sc_hb_to_I_NTid detour_E.
rewrite wf_new_rfE.
arewrite (⦗E⦘ ⊆ ⦗E \₁ I⦘ ∪ ⦗E ∩₁ I⦘).
...
generalize (wf_new_rfl); clear; basic_solver.
...
{ generalize R_Acq_codom_rfe. basic_solver 12. }
...
Proof using OLD_VAL SAME TCCOH.
...
Proof using ACYC_EXT COH COMP_NTID CSC G Grfe_E IT_new_co T TCCOH TCCOH_rst_new_T WF WF_SC
         W_hb_sc_hb_to_I_NTid detour_E.
unfold Execution.rfe; ins.
rewrite cert_sb.
split; [|clear; basic_solver 12].
...
Proof using ACYC_EXT COH COMP_NTID CSC Grfe_E IT_new_co TCCOH TCCOH_rst_new_T WF WF_SC
      W_hb_sc_hb_to_I_NTid detour_E.
rewrite cert_rfe.
...
Proof using WF WF_SC. ins; split; [rewrite wf_new_rfE|]; clear; basic_solver 20. Qed.
...
Proof using SAME WF WF_SC W_ex_E.
...
Proof using COH COMP_ACQ CSC IT_new_co TCCOH WF WF_SC.
unfold new_rf.
unfolder; ins; desf.
...
exploit (new_co_I IT_new_co); try apply WF; [| clear; basic_solver].
...
Proof using ACYC_EXT COH COMP_ACQ COMP_NTID CSC E_to_I G IT_new_co SAME T TCCOH
      TCCOH_rst_new_T WF WF_SC W_ex_E W_hb_sc_hb_to_I_NTid detour_E.
...
- arewrite (Gsb ⨾ ⦗F⦘ ⨾ ⦗Acq⦘ ≡ ⦗D⦘ ⨾ Gsb ⨾ ⦗F⦘ ⨾ ⦗Acq⦘).
  by rewrite (dom_r (@wf_sbE G)); generalize dom_sb_F_Acq_in_D; clear; basic_solver 12.
  clear; basic_solver 12.
...
- arewrite (Gsb ⨾ ⦗F⦘ ⨾ ⦗Acq⦘ ≡ ⦗D⦘ ⨾ Gsb ⨾ ⦗F⦘ ⨾ ⦗Acq⦘).
  by rewrite (dom_r (@wf_sbE G)); generalize dom_sb_F_Acq_in_D; clear; basic_solver 12.
  rewrite (dom_r wf_new_rfE); clear; basic_solver.
- clear; basic_solver 12.
- rewrite rfi_union_rfe; relsf; unionL.
  2: by rewrite <- R_Acq_codom_rfe at 2; rewrite (dom_r (wf_rfeD WF)) at 1; clear; basic_solver 21.
  arewrite (Grelease ⊆ Gsb^? ∪ Grelease ⨾ ⦗GW_ex⦘) at 1.
  { unfold imm_s_hb.release, imm_s_hb.rs.
    rewrite rtE at 1; relsf; unionL.
    generalize (@sb_trans G); clear; basic_solver 12.
    rewrite rmw_W_ex at 1.
    rewrite <- !seqA, inclusion_ct_seq_eqv_r, !seqA.
    clear; basic_solver 21. }
  relsf; unionL.
  by arewrite (Grfi ⊆ Gsb); generalize (@sb_trans G); clear; basic_solver 12.
  rewrite <- R_Acq_codom_W_ex_rfi at 1; rewrite (dom_r (wf_rfiD WF)) at 1; clear; basic_solver 21.
- arewrite (Gsb ⨾ ⦗F⦘ ⨾ ⦗Acq⦘ ≡ ⦗D⦘ ⨾ Gsb ⨾ ⦗F⦘ ⨾ ⦗Acq⦘).
  by rewrite (dom_r (@wf_sbE G)); generalize dom_sb_F_Acq_in_D; clear; basic_solver 12.
  clear; basic_solver 21.
Qed.
...
{ arewrite (⦗C⦘ ≡ ⦗D⦘ ⨾ ⦗C⦘).
split.
generalize C_in_D; clear; basic_solver.
clear; basic_solver. 
rewrite !crE; relsf. 
seq_rewrite cert_rf_D.
rewrite !seqA.
...
all: try by apply WF.
- apply dom_helper_3; rewrite (wf_rfE WF), wf_new_rfE; clear; basic_solver.
- apply dom_helper_3; rewrite (wf_rfD WF), wf_new_rfD; clear; basic_solver.
- rewrite (wf_rfl WF), wf_new_rfl; clear; basic_solver.
...
- rewrite transp_union; apply functional_union.
  by unfolder; ins; eapply (wf_rff WF); clear; basic_solver.
  by apply wf_new_rff.
...
{ ie_unfolder; unfolder; ins; desc; splits; eauto.
  ...
  all: generalize (init_issued WF TCCOH); basic_solver. }
...
- rewrite cert_rfe; relsf; unionL.
  * revert COH CSC; unfold coherence, coh_sc, eco; ie_unfolder; clear; basic_solver 21.
  * generalize new_rf_hb; clear; basic_solver 12.
- rewrite cert_co_alt'; relsf; unionL.
  * revert COH CSC; unfold coherence, coh_sc, eco; clear; basic_solver 21.
  * revert W_hb_sc_hb_to_I_NTid; clear; basic_solver 21.
- rewrite cert_rfe; relsf; unionL.
  ...
  * rewrite cert_co_alt'; relsf; unionL.
    + rewrite new_rf_in_furr.
      ...
      by generalize (furr_hb_sc_hb WF WF_SC ACYC_EXT); clear; basic_solver 21.
      generalize (eco_furr_irr WF WF_SC CSC COH).
      unfold eco; clear; basic_solver 21.
    + generalize non_I_new_rf; clear; basic_solver 16.
- unfold fr; rewrite cert_co_alt'; unfold certG; ins.
  rewrite transp_union, transp_seq; relsf; unionL.
  * revert COH CSC; unfold coherence, coh_sc, eco, fr; ie_unfolder; clear; basic_solver 21.
  * rotate 1.
    arewrite (Gco ∩ cert_co ⊆ cert_co).
    rewrite (dom_r (wf_cert_coD)), !seqA.
    arewrite (⦗W⦘ ⨾ Ghb ⨾ (sc ⨾ Ghb)^? ⊆ Gfurr).
    rewrite (furr_alt WF_SC); clear; basic_solver 21.
    unfold new_rf; clear; basic_solver 21.
  * rewrite !seqA; eapply cert_coherece_detour_helper.
  * rotate 1.
    arewrite (⦗E ∩₁ W ∩₁ Tid_ thread \₁ I⦘ ⊆ ⦗W⦘) by clear; basic_solver.
    arewrite (⦗W⦘ ⨾ Ghb ⨾ (sc ⨾ Ghb)^? ⊆ Gfurr).
    rewrite (furr_alt WF_SC); clear; basic_solver 21.
    unfold new_rf; clear; basic_solver 21.
- rewrite cert_rfe; relsf; unionL.
  * unfold fr; unfold certG; ins.
    ...
    + rewrite (dom_rel_helper Grfe_E), !seqA.
      sin_rewrite cert_co_I.
      revert COH CSC; unfold coherence, coh_sc, eco, fr; ie_unfolder; clear; basic_solver 21.
    + arewrite (Grfe ⨾ ⦗D⦘ ⊆ Grf) by ie_unfolder; clear; basic_solver.
      rotate 1.
      arewrite (Grf ⨾ Ghb ⨾ (sc ⨾ Ghb)^? ⊆ Gfurr).
      rewrite (furr_alt WF_SC); rewrite (dom_l (wf_rfD WF)); clear; basic_solver 21.
      unfold new_rf; clear; basic_solver 21.
  * unfold fr; unfold certG; ins.
    rewrite transp_union, transp_seq; relsf; unionL.
    + rewrite cert_co_alt'; relsf; unionL.
      -- rewrite new_rf_in_furr.
         ...
         by generalize (furr_hb_sc_hb WF WF_SC ACYC_EXT); clear; basic_solver 21.
         generalize (eco_furr_irr WF WF_SC CSC COH).
         unfold eco, fr; clear; basic_solver 21.
      -- generalize non_I_new_rf; clear; basic_solver 16.
    + rewrite cert_co_alt'; relsf; unionL.
...
Proof using All.
red; generalize coh_helper; clear; basic_solver 12.
Qed.
...
- rewrite cert_hb.
  ...
  eapply t_trans; [edone| apply t_step].
  by apply sc_in_ar.
- generalize coh_helper; clear; basic_solver 21.
Qed.
...
relsf; unionL; [| clear; basic_solver].
unfold cert_co.
...
relsf; unionL.
1,2: generalize (co_trans WF); revert AT'; unfold fr; clear; basic_solver 12.
...
Proof using All.
  ...
  { unfold ppo; rewrite cert_R, cert_W, cert_sb, cert_R_ex.
    rewrite HeqX; hahn_frame; apply inclusion_t_t; clear; basic_solver 12. }
  arewrite (Gppo ≡ ⦗R⦘ ⨾ (X ∪ Grfi)⁺ ⨾ ⦗W⦘).
  { unfold ppo. rewrite HeqX. split; hahn_frame; apply inclusion_t_t; clear; basic_solver 12. }
  arewrite (⦗W⦘ ⨾ ⦗D⦘ ⊆ ⦗D⦘ ⨾ ⦗W⦘) by clear; basic_solver.
  hahn_frame.
  rewrite path_union; relsf; unionL.
  generalize inclusion_t_t; clear; basic_solver.
  rewrite !seqA.
  ...
  { rewrite HeqX.
    relsf; unionL; splits.
    { apply dom_data_D. }
    { rewrite (dom_rel_helper dom_ctrl_in_D). clear; basic_solver. }
    { rewrite (dom_rel_helper dom_addr_in_D). clear; basic_solver. }
    { rewrite (dom_rel_helper dom_rmw_in_D). clear; basic_solver. }
    { rewrite (dom_rel_helper dom_frmw_in_D). clear; basic_solver. }
    rewrite (dom_rel_helper dom_Rex_sb_in_D). clear; basic_solver. }
  ...
  { sin_rewrite H; arewrite (X ⊆ (X ∪ Grfi)⁺) at 2; relsf; clear; basic_solver. }
...
- arewrite (⦗D⦘ ⊆ ⦗D⦘ ⨾ ⦗D⦘) by clear; basic_solver.
sin_rewrite H.
...
arewrite (⦗codom_rel (⦗I⦘ ⨾ Grfe ⨾ ⦗D⦘)⦘ ⊆ ⦗D⦘) by clear; basic_solver.
sin_rewrite cert_detour_D; clear; basic_solver 40.
...
Proof using All.
(* case_refl _; [clear; basic_solver|]. *)
rewrite !seqA.
arewrite (⦗W⦘ ⨾ ⦗C ∪₁ I⦘ ⊆ ⦗W⦘ ⨾ ⦗I⦘).
generalize (w_covered_issued TCCOH); clear; basic_solver.
generalize (dom_W_Rel_sb_loc_I_in_C TCCOH); clear; basic_solver 12.
Qed.
...
exploit COMP_ACQ; [clear; basic_solver|].
intros [w A]; desf.
...
rewrite <- ct_step; clear; basic_solver 12.
rewrite (dom_l (@wf_sbE G)) at 2.
generalize E_F_AcqRel_in_C, (dom_sb_covered TCCOH), C_in_D.
rewrite ct_begin, <- inclusion_t_rt, <- ct_step; clear; basic_solver 32.
- unfold ar_int, bob.
rewrite <- ct_step; clear; basic_solver 12.
- unfold ar_int, bob, fwbob.
case_refl Gsb.
...
generalize E_F_AcqRel_in_C, (dom_sb_covered TCCOH), C_in_D.
rewrite <- ct_step; clear; basic_solver 21.
rewrite (dom_r (@wf_sbE G)) at 1.
generalize E_F_AcqRel_in_C, (dom_sb_covered TCCOH), C_in_D.
rewrite ct_begin, <- inclusion_t_rt, <- ct_step; clear; basic_solver 32.
- unfold ar_int, bob, fwbob.
...
generalize C_in_D, I_in_D.
rewrite <- ct_cr, <- ct_step; clear; basic_solver 32.

- rewrite !seqA.
rewrite (cr_helper W_rel_sb_loc_W_CI).

arewrite ((⦗W ∩₁ Rel⦘ ⨾ Gsb ∩ Gsame_loc ⨾ ⦗W⦘)^?  ⊆ Gar_int^?) at 1.
unfold ar_int, bob, fwbob; clear; basic_solver 21.

rewrite <- (ct_cr Gar_int).
hahn_frame_r.

arewrite (Cppo ⨾ ⦗C ∪₁ I⦘ ⊆ Gppo⨾ ⦗C ∪₁ I⦘).
generalize cert_ppo_CI; clear; basic_solver 12.

arewrite (Gppo ⨾ ⦗C ∪₁ I⦘ ⊆ ⦗D⦘ ⨾ Gppo).
rewrite C_in_D, I_in_D; generalize dom_ppo_D; clear; basic_solver.


rewrite <-ct_step.

unfold ar_int; clear; basic_solver 12.
...
by unfold ar_int, bob, fwbob; clear; basic_solver 21.
...
generalize W_ex_E; clear; basic_solver 21.
...
by generalize (W_ex_not_init WF); clear; basic_solver.
...
Proof using All.
rewrite path_union.
generalize (sc_trans WF_SC); ins; relsf; unionL; [clear; basic_solver|].
rewrite crE at 2; relsf; unionL.
-
arewrite (sc^? ⨾ rfe certG ⊆ rfe certG ).
rewrite crE; relsf; unionL; [clear; basic_solver|].
rewrite (dom_l (wf_rfeD WF_cert)), cert_W.
rewrite (dom_r (wf_scD WF_SC)) at 1.
clear; type_solver.

rewrite ct_begin, rtE; relsf; unionL; [clear; basic_solver|].
rewrite ct_begin.
...
arewrite (Sc ⊆₁ Acq/Rel) by clear; mode_solver.
generalize E_F_AcqRel_in_C; clear; basic_solver 12.
- rewrite cert_rfe; clear; basic_solver 12.
}
...
{ rewrite cert_rfe; relsf; unionL; [clear; basic_solver 12|].
...
- rewrite sb_W_rel_CI; clear; basic_solver.
- rewrite W_rel_sb_loc_W_CI; clear; basic_solver.
- rewrite (dom_r (@wf_sbE G)).
generalize dom_sb_F_AcqRel_in_CI. clear; basic_solver 12.
- rewrite (dom_l (@wf_sbE G)).
generalize E_F_AcqRel_in_C; clear; basic_solver 12.
Qed.
...
Proof using All.
  ...
  { arewrite (sb certG ⨾ ⦗R ∩₁ Acq⦘ ⨾ sb certG ⊆ sb certG).
    2: by rewrite ACQEX.
    generalize (@sb_trans certG). clear; basic_solver. }
  ...
  { rewrite rfi_union_rfe; relsf; unionL; splits; ins.
    ...
    arewrite (Crfi ⊆ Gsb).
    generalize tc_sb_C, tc_W_C_in_I; clear; basic_solver 21.
    rewrite cert_rfe; clear; basic_solver 21. }
  { ins; rewrite cert_W; done. }
  { ins; rewrite cert_fwbob; done. }
  2: { ins. by rewrite cert_W_ex_acq. }
  relsf; unionL; splits; simpls.
  3,4: rewrite cert_rfe; clear; basic_solver.
  { rewrite I_in_D at 1.
    arewrite (⦗D⦘ ⊆ ⦗D⦘ ⨾ ⦗D⦘) by clear; basic_solver.
  ...
  sin_rewrite cert_rfi_D.
  arewrite_id ⦗D⦘. rewrite !seq_id_l.
  arewrite (Grfi ⊆ Grf).
  eapply rf_ppo_loc_I_in_I; eauto.
Qed.

Lemma dom_cert_ar_rf_ppo_loc_I : dom_rel (⦗is_w certG.(lab)⦘ ⨾ (Car ∪ Crf ⨾ Cppo ∩ (same_loc certG.(lab)))⁺ ⨾ ⦗I⦘) ⊆₁ I.
Proof using All.
...
Proof using All.
  ...
  { ins; rewrite cert_fwbob; done. }
  ins.
  apply dom_cert_ar_rf_ppo_loc_I.
Qed.
